...
class ImgManager extends CApplicationComponent
{
	 ...
	 * 
	 */
	public function saveUrl($url,$name,$path=null){
		$trx=Yii::app()->db->beginTransaction();
		$pathRuntime = Yii::getPathOfAlias('application.runtime');
		$imgTemp = $pathRuntime.DIRECTORY_SEPARATOR.'imagetemp'.DIRECTORY_SEPARATOR.$name.".jpg";
		try
		{	
			$content = @file_get_contents($url);
			if($content == false){
				throw new ImgException(Img::t('error','Content Tidak Ditemukan.'));
			}
			@file_put_contents($imgTemp,$content);
			if(!file_exists($imgTemp)){
				throw new ImgException(Img::t('error','File Gagal Disimpan.'));
			}
			$propertis = getimagesize ( $imgTemp );
			$file = new CUploadedFile($name.".jpg",$imgTemp,$propertis['mime'],filesize($imgTemp),0);
			$image=new Image();
			$image->extension=strtolower($file->getExtensionName());
			$image->filename=$file->getName();
			$image->byteSize=$file->getSize();
